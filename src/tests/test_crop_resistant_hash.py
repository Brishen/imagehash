...
class Test(TestImageHash):
	...
	def test_segmented_hash__hash_func(self):
		segmented_ahash = imagehash.crop_resistant_hash(
			self.image, imagehash.average_hash
		)
	...
	def test_segmented_hash__limit_segments(self):
		...
		self.assertGreaterEqual(
			len(segmented_orig.segment_hashes),
			len(segmented_limit.segment_hashes),
	...
	def test_segmented_hash__segment_threshold(self):
		segmented_low_threshold = imagehash.crop_resistant_hash(
			self.image, segment_threshold=20
		)
		segmented_high_threshold = imagehash.crop_resistant_hash(
			self.image, segment_threshold=250
		)
		self.assertFalse(
	...
	def test_segmentation_image_size(self):
		...
		self.assertGreater(
			large_timed,
			small_timed,
			'Hashing should take longer when the segmentation image is larger',
		)

	def test_min_segment_size(self):
		small_segments_hash = imagehash.crop_resistant_hash(
			self.peppers, min_segment_size=100
		)
		big_segments_hash = imagehash.crop_resistant_hash(
			self.peppers, min_segment_size=1000
		)
	...
	def test_crop_resistance(self):
		...
		crop_10 = full_image.crop(
			(0.05 * width, 0.05 * height, 0.95 * width, 0.95 * height)
		)
		crop_40 = full_image.crop(
		...
		self.assertEqual(
			crop_hash_10,
			full_hash,
			'Slightly cropped image hash should match full image hash',
		)
		self.assertEqual(
			crop_hash_40,
			full_hash,
			'Heavily cropped image hash should match full image hash',
		)
		self.assertEqual(
			crop_hash_asymmetric,
			full_hash,
